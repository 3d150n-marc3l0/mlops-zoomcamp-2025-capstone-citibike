...
def citibike_training_pipeline(
    ...
    logger.info(
        f"start_train_date: {start_train_date}, test_size: {end_train_date}"
    )
    ...
    raw_data, target = data_loader(
        base_url=data_url, start_date=start_train_date, end_date=end_train_date
    )
    ...
    best_params = optimize_hyperparams(
        train, target, n_trials=n_trials, after=["evidently_test_step"]
    )

    ########## Training stage ##########

    model = xgb_trainer(train, target, best_params)

    test_metrics = model_scorer(model, train, target)
