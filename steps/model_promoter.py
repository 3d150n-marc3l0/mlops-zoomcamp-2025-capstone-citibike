...
from utils.promotion_helper import (
...
def promote_models(
    model_name: str = "xgb-citibike-reg-model", threshold: float = 0.2
) -> Annotated[bool, "is_promoted"]:
    logger.info("Prompte model")
    ...
    candidate_info = client.get_model_version_by_alias(
        name=model_name, alias=CANDIDATE_MODEL
    )
    logger.info(
        f"{CANDIDATE_MODEL} model: {candidate_info} {type(candidate_info.version)}"
    )
    ...
    try:
        champion_info = client.get_model_version_by_alias(
            name=model_name, alias=CHAMPION_MODEL
        )
        logger.info(
            f"{CHAMPION_MODEL} model: {champion_info} {type(champion_info.version)}"
        )
        ...
        logger.info(f"Candidate improvement : {improvement}")
        ...
        if improvement >= threshold:
            ...
            logger.info(
                f"Candidate promoted to Production. Candidate model ({candidate_info.version}) has lower MAE ({candidate_rmse}) than current production ({production_rmse})."
            )
            is_promoted = True
        else:
            ...
            reject_candidate_in_model_registry(
                model_name=model_name, current_version=candidate_info.version
            )
        ...
        logger.info("Completed: Auto Promotion of Staging to Production")
        is_promoted = True
...
def get_model_by_alias(
    ...
    logger.info(
        f"Retrieve info model {model_name} with alias {model_alias}, model: {type(model)}"
    )
    return model
